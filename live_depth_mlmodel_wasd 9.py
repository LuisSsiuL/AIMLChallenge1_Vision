...
# --- Gamma Correction ---
NAVIGATION_GAMMA      = 0.6    # Applied to depth values before zone scoring (< 1 = brighten/compress)
...
STATIC_STUCK_DIFF_THRESHOLD       = 1.2  # mean abs pixel diff below this counts as "no change"
...
class Navigator:
    ...
    def process_frame(self, depth_u8: np.ndarray) -> tuple:
        ...
        if self._prev_motion_depth is not None and self._prev_motion_depth.shape == motion_view.shape:
            ...
            if mean_motion_diff < STATIC_STUCK_DIFF_THRESHOLD:
        ...
        must_reverse_now = (
            ...
            or self._static_stuck_frames >= STATIC_STUCK_CONFIRM_FRAMES
        )
        ...
        if must_reverse_now:
            ...
            if self._consecutive_reverse > REVERSE_ESCAPE_FRAMES:
                left_avg = sum(near_scores[:3]) / 3.0
                right_avg = sum(near_scores[4:]) / 3.0
                raw_cmd = (
                    DrivingCommand.REVERSE_LEFT
                    if left_avg >= right_avg
