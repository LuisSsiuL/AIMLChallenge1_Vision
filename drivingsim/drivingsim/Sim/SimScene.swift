...
final class SimScene {
    ...
    private let maxSteerRate:  Float = 1.8       // up from 1.2 — sharper turns
    private let steerSpeedRef: Float = 0.08      // down from 0.3 — full steer kicks in at very low speed
    ...
    private struct Obstacle { let pos: SIMD3<Float>; let halfX: Float; let halfZ: Float; let height: Float; var isPerson: Bool = false; var color: NSColor = NSColor(white: 0.55, alpha: 1) }
    ...
    var obstacleSnapshot: [(pos: SIMD3<Float>, halfX: Float, halfZ: Float, height: Float, color: NSColor)] {
        obstacles.filter { !$0.isPerson }.map { ($0.pos, $0.halfX, $0.halfZ, $0.height, $0.color) }
    }
    ...
    private func setupOfficeRoom() {
        ...
        let wallColor    = NSColor(red: 0.78, green: 0.85, blue: 0.92, alpha: 1)   // soft cool blue
        ...
        addBox(at: [-halfW - wallT/2, wallH/2, 0], w: wallT, h: wallH, d: roomD, color: wallColor)
        // South wall — solid (no doorway). Fully enclosed room.
        addBox(at: [0, wallH/2, halfD + wallT/2], w: roomW + wallT*2, h: wallH, d: wallT, color: wallColor)
    ...
    private func addBox(at pos: SIMD3<Float>, w: Float, h: Float, d: Float, color: NSColor) {
        ...
        ent.position = pos
        root.addChild(ent)
        obstacles.append(Obstacle(pos: pos, halfX: w / 2, halfZ: d / 2, height: h, color: color))
    }
    ...
    private func addLights() {
        SimSceneLighting.attach(to: root, addShadowKey: true)
    }
    ...
    private func tick(kb: KeyboardMonitor,
        ...
        //   .autoMap     — composite. While exploring/approachingTarget the
        //                  autoSeekPy controller (DepthDriver 5-zone + YOLOPy
        ...
        case .autoSeekPy, .autoMap, .mapMetric, .mapExplore:
        ...
        let inRoam = seekPyActive && activeYoloSeekState == .roam
        let inSeek = seekPyActive && activeYoloSeekState == .seek

        // YOLO seek steering law.
        // `nearTarget`: bbox fills ≥ 60% of frame height → close enough,
        // brake hard. Suppresses depth-driver forward too so the car doesn't
        // ram the person.
        ...
        // autoSeekPy depth gating — DepthDriver controls all 4 axes when
        // either standalone autoSeekPy OR autoMap-while-seeking.
        let depthSeekAll = seekPyActive

        // Near-target hard brake gates ALL forward sources so the car cannot
        // continue forward by either seek bias or depth zone-decision.
        let allowForward = !nearTarget
        let inForward  = allowForward && (
                           (kbActive && kb.forward)  || (handActive && hand.forward)
                           || (depthWS && depth.forward)
                           || (depthSeekAll && inRoam && depth.forward)
                           || yoloFwd
                           || (mapDriving && map.forward))
        let inBackward = (kbActive && kb.backward) || (handActive && hand.backward)
                         || (depthWS && depth.backward)
                         || (depthSeekAll && inRoam && depth.backward)
                         || (mapDriving && map.backward)
        let inLeft     = (kbActive && kb.left)     || (handActive && hand.left)
                         || (depthAD && depth.left)
                         || (depthSeekAll && inRoam && depth.left)
                         || yoloLeft
                         || (mapDriving && map.left)
        let inRight    = (kbActive && kb.right)    || (handActive && hand.right)
                         || (depthAD && depth.right)
                         || (depthSeekAll && inRoam && depth.right)
                         || yoloRight
                         || (mapDriving && map.right)
        ...
        // ── Steering ──
        // Diff-drive / skid-steer: pure rotation when A/D pressed without W/S.
        // Ackermann-style (speed-coupled) used otherwise.
        let steerInput: Float = (inRight ? 1 : 0) + (inLeft ? -1 : 0)
        let inPlaceRotate = (!inForward && !inBackward && steerInput != 0)
        let yawRate: Float
        if inPlaceRotate {
            yawRate = steerInput * maxSteerRate           // full rate, in-place
        } else {
        ...
        // Push truth pose to MapDriver every tick — replaces dead-reckoning so
        // map↔world stays aligned regardless of collision damping.
        map.updateTruthPose(SIMD2<Float>(car.position.x, car.position.z), yaw: yaw)
