...
struct SourcePickerView: View {
    ...
    private func rescan() {
        var list = CameraScanner.discover()
        list.append(.esp32(host: ESP32Config.host))
        sources = list
    ...
    private func iconName(for src: CameraSource) -> String {
        ...
        case .live(_, let name):
            ...
            return "camera"
        case .esp32: return "antenna.radiowaves.left.and.right"
        }
    }
    ...
    private func subtitle(for src: CameraSource) -> String {
        ...
        case .esp32(let host): return "MJPEG http://\(host)/stream · WS ws://\(host):\(ESP32Config.wsPort)"
        }
    }
}
