...
enum DrivingCommand: String {
    ...
    case reverseRight
    case brake
    case rotateLeft     // pure spin in place (diff-drive)
    case rotateRight
    ...
    var label: String {
        ...
        case .rotateLeft:   return "<. ROT LEFT"
        case .rotateRight:  return ".> ROT RIGHT"
        }
    }
}
...
final class DepthDriver: ObservableObject {
    ...
    // Raw metric meters snapshot (only populated when `publishMeters` true).
    // Width/height match model output. MapDriver uses this for ≤5m occupancy projection.
    struct MetricFrame {
        let meters: [Float]
        let width:  Int
        let height: Int
        let frameID: UInt64
        /// Pose at time the source pixel buffer was rendered (passed to submit).
        /// Used by consumer to project using submit-time yaw, not consume-time yaw.
        let posePos: SIMD2<Float>
        let poseYaw: Float
    }
    @Published private(set) var latestMetric: MetricFrame?
    /// Enable raw float meters output. Off by default — costs a second pass.
    /// Thread-safe via lock — readable from inference queue.
    nonisolated private let publishMetersLock = OSAllocatedUnfairLock<Bool>(initialState: false)
    func setPublishMeters(_ on: Bool) { publishMetersLock.withLock { $0 = on } }
    nonisolated(unsafe) private var metricFrameCounter: UInt64 = 0
    ...
    private func loadModel() {
        ...
        let url: URL
        if let u = Bundle.main.url(forResource: modelName, withExtension: "mlmodelc")
                ?? Bundle.main.url(forResource: modelName, withExtension: "mlpackage") {
            url = u
            print("[DepthDriver] model found: \(modelName)")
        } else {
            print("[DepthDriver] FATAL: depth model not bundled (\(modelName)) — driver will not run")
            return
        }
        do {
    ...
    /// Pose is captured at submit time so the projected occupancy uses the
    /// yaw of the rendered frame, not whatever pose exists at consume time.
    nonisolated func submit(_ pixelBuffer: CVPixelBuffer,
                            posePos: SIMD2<Float> = .zero,
                            poseYaw: Float = 0)
    {
        ...
        visionQueue.async { [weak self] in
            self?.runInference(pixelBuffer, posePos: posePos, poseYaw: poseYaw)
        }
    }
    ...
    nonisolated private func runInference(_ pixelBuffer: CVPixelBuffer,
        ...
        var metricSnapshot: MetricFrame? = nil
        let wantMeters: Bool = publishMetersLock.withLock { $0 }
        if wantMeters {
            if let (meters, mw, mh) = Self.depthBufferToFloats(outBuf) {
                metricFrameCounter &+= 1
                metricSnapshot = MetricFrame(meters: meters, width: mw, height: mh,
                                             frameID: metricFrameCounter,
        ...
        Task { @MainActor [self] in
            ...
            if let snap = metricSnapshot { self.latestMetric = snap }
    ...
    /// Raw float extraction — no normalize, no invert.
    /// Returns meters (for metric models) or arbitrary scale (for relative models).
    /// Caller is responsible for knowing which.
    nonisolated static func depthBufferToFloats(_ buf: CVPixelBuffer) -> ([Float], Int, Int)? {
        CVPixelBufferLockBaseAddress(buf, .readOnly)
        defer { CVPixelBufferUnlockBaseAddress(buf, .readOnly) }
        let w = CVPixelBufferGetWidth(buf)
        let h = CVPixelBufferGetHeight(buf)
        ...
        case kCVPixelFormatType_OneComponent16Half:
            for y in 0..<h {
                let row = base.advanced(by: y * stride).assumingMemoryBound(to: UInt16.self)
                for x in 0..<w { floats[y * w + x] = Float(Float16(bitPattern: row[x])) }
            }
        case kCVPixelFormatType_OneComponent32Float:
            for y in 0..<h {
                let row = base.advanced(by: y * stride).assumingMemoryBound(to: Float.self)
                for x in 0..<w { floats[y * w + x] = row[x] }
            }
        case kCVPixelFormatType_OneComponent8:
            for y in 0..<h {
                let row = base.advanced(by: y * stride).assumingMemoryBound(to: UInt8.self)
                for x in 0..<w { floats[y * w + x] = Float(row[x]) / 255.0 }
            }
        default:
            for y in 0..<h {
                let row = base.advanced(by: y * stride).assumingMemoryBound(to: UInt16.self)
